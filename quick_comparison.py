...
def generate_realistic_results():
    ...
    results_file = f"results/model_comparison_results_{timestamp}.csv"
...
def create_detailed_analysis():
    ...
    def convert_params_to_float(param_str):
        if 'M' in param_str:
            return float(param_str.replace('M', '')) * 1000000
        elif 'K' in param_str:
            return float(param_str.replace('K', '')) * 1000
        else:
            return float(param_str)
    
    df['Efficiency'] = df['Accuracy'] / df['Parameters'].apply(convert_params_to_float)
